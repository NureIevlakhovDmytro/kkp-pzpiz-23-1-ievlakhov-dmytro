...
describe('Products (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    unitId = (
      await request(app.getHttpServer())
        .post('/api/units')
        .set(auth())
        .send({ code: `pu${Date.now()}`, name: 'kg' })
    ).body.id;
    categoryId = (
      await request(app.getHttpServer())
        .post('/api/categories')
        .set(auth())
        .send({ name: `PCat-${Date.now()}` })
    ).body.id;
  });
  afterAll(async () => {
    await app.close();
  ...
  it('creates a product referencing an active unit + category', async () => {
    const res = await request(app.getHttpServer())
      .post('/api/products')
      .set(auth())
      .send({ name: `Prod-${Date.now()}`, unitId, categoryId, minStock: 5 })
      .expect(201);
  ...
  it('rejects creating a product with an unknown unit (404)', async () => {
    await request(app.getHttpServer())
      .post('/api/products')
      .set(auth())
      .send({ name: 'X', unitId: '00000000-0000-0000-0000-000000000000' })
      .expect(404);
  });
  ...
  it('rejects creating a product against an archived category (422)', async () => {
    ...
    const res = await request(app.getHttpServer())
      .post('/api/products')
      .set(auth())
      .send({ name: 'Y', unitId, categoryId: c.body.id })
      .expect(422);
  ...
  it('blocks archiving a category still used by an active product (409)', async () => {
    const cat = (
      await request(app.getHttpServer())
        .post('/api/categories')
        .set(auth())
        .send({ name: `Used-${Date.now()}` })
    ).body;
    await request(app.getHttpServer())
      .post('/api/products')
      .set(auth())
      .send({ name: `P2-${Date.now()}`, unitId, categoryId: cat.id })
      .expect(201);
    const res = await request(app.getHttpServer())
