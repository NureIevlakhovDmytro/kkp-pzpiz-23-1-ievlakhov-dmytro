...
describe('Inventory ★ recompute-at-complete (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    unitId = (
      await request(app.getHttpServer())
        .post('/api/units')
        .set(auth())
        .send({ code: `iv${tag}`, name: 'kg' })
    ).body.id;
    productId = (
      await request(app.getHttpServer())
        .post('/api/products')
        .set(auth())
        .send({ name: `IV-Prod-${tag}`, unitId })
    ).body.id;
    locationId = (
      await request(app.getHttpServer())
        .post('/api/storage-locations')
        .set(auth())
        .send({ name: `IV-Loc-${tag}` })
    ).body.id;
    currencyId = (
    ...
    const receipt = await request(app.getHttpServer())
      ...
      .send({
        locationId,
        date: '2026-05-30',
        lines: [{ productId, batchNumber: `IB-${tag}`, quantity: 100, unitCost: 10, currencyId }],
      });
  ...
  it('creates a DRAFT inventory that snapshots current stock as expected', async () => {
    const res = await request(app.getHttpServer())
      ...
      .set(auth())
      .send({ locationId, date: '2026-05-31' })
      .expect(201);
  ...
  it('complete fails while a line is uncounted (409)', async () => {
    const inv = (
      await request(app.getHttpServer())
        .get(`/api/inventory-counts?locationId=${locationId}&status=DRAFT`)
        .set(auth())
    ).body.items[0];
    await request(app.getHttpServer())
  ...
  it('counts a shortage and completes: stock set to actual, adjustment records SHORTAGE delta', async () => {
    ...
    await request(app.getHttpServer())
      ...
      .set(auth())
      .send({ counts: [{ batchId, actualQty: 90 }] })
      .expect(200);
