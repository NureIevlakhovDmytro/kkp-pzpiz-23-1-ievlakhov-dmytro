...
describe('Reports ★ loss structure + inventory (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    unitId = (
      await request(app.getHttpServer())
        .post('/api/units')
        .set(auth())
        .send({ code: `rp${tag}`, name: 'kg' })
    ).body.id;
    productId = (
      await request(app.getHttpServer())
        .post('/api/products')
        .set(auth())
        .send({ name: `RP-Prod-${tag}`, unitId })
    ).body.id;
    locationId = (
      await request(app.getHttpServer())
        .post('/api/storage-locations')
        .set(auth())
        .send({ name: `RP-Loc-${tag}` })
    ).body.id;
    currencyId = (
    ...
    const receipt = await request(app.getHttpServer())
      ...
      .send({
        locationId,
        date: '2026-05-10',
        lines: [{ productId, batchNumber: `RB-${tag}`, quantity: 100, unitCost: 10, currencyId }],
      });
    ...
    await request(app.getHttpServer())
      ...
      .send({
        ...
        lines: [{ batchId, locationId, quantity: 6 }],
      });
  });
  afterAll(async () => {
    await app.close();
  ...
  it('loss-structure aggregates spoilage in base currency', async () => {
    const res = await request(app.getHttpServer())
      .get('/api/reports/loss-structure?from=2026-05-01&to=2026-05-31')
      .set(auth())
      .expect(200);
  ...
  it('loss-structure excludes a reversed write-off', async () => {
    const wo = await request(app.getHttpServer())
      ...
      .send({
        date: '2026-05-13',
        reasonId: spoilageReasonId,
        lines: [{ batchId, locationId, quantity: 4 }],
      });
    const beforeRows = (
      await request(app.getHttpServer())
        .get('/api/reports/loss-structure?from=2026-05-01&to=2026-05-31')
    ...
    await request(app.getHttpServer())
      .post(`/api/write-offs/${wo.body.id}/reverse`)
      .set(auth())
      .expect(201);
    const afterRows = (
      await request(app.getHttpServer())
  ...
  it('inventory report reflects a counted shortage', async () => {
    const inv = (
      await request(app.getHttpServer())
        ...
        .set(auth())
        .send({ locationId, date: '2026-05-20' })
    ).body;
    await request(app.getHttpServer())
      .patch(`/api/inventory-counts/${inv.id}`)
      .set(auth())
      .send({ counts: [{ batchId, actualQty: 80 }] })
      .expect(200);
    await request(app.getHttpServer())
      .post(`/api/inventory-counts/${inv.id}/complete`)
      .set(auth())
      .expect(201);
    const report = await request(app.getHttpServer())
      .get(`/api/reports/inventory/${inv.id}`)
      .set(auth())
      .expect(200);
