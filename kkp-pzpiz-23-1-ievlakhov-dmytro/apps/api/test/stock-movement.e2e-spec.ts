...
describe('Stock-movement report (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    unitId = (await request(app.getHttpServer()).post('/api/units').set(auth()).send({ code: `mv${tag}`, name: 'kg' })).body.id;
    productId = (await request(app.getHttpServer()).post('/api/products').set(auth()).send({ name: `MV-Prod-${tag}`, unitId })).body.id;
    fromId = (await request(app.getHttpServer()).post('/api/storage-locations').set(auth()).send({ name: `MV-From-${tag}` })).body.id;
    toId = (await request(app.getHttpServer()).post('/api/storage-locations').set(auth()).send({ name: `MV-To-${tag}` })).body.id;
    ...
    const receipt = await request(app.getHttpServer()).post('/api/receipts').set(auth())
      .send({ locationId: fromId, date: '2026-05-15', lines: [{ productId, batchNumber: `MVB-${tag}`, quantity: 40, unitCost: 10, currencyId }] });
    batchId = receipt.body.lines[0].batchId;
    await request(app.getHttpServer()).post('/api/transfers').set(auth())
      .send({ fromLocationId: fromId, toLocationId: toId, date: '2026-05-16', lines: [{ batchId, quantity: 15 }] });
    await request(app.getHttpServer()).post('/api/write-offs').set(auth())
      .send({ date: '2026-05-17', reasonId: spoilageReasonId, lines: [{ batchId, locationId: fromId, quantity: 5 }] });
  });
  afterAll(async () => { await app.close(); });

  it('reconstructs the movement ledger for the product', async () => {
    const res = await request(app.getHttpServer())
      .get(`/api/reports/stock-movement?from=2026-05-01&to=2026-05-31&productId=${productId}`).set(auth()).expect(200);
    const types = res.body.map((r: any) => r.documentType);
    expect(types).toEqual(expect.arrayContaining(['RECEIPT', 'TRANSFER_OUT', 'TRANSFER_IN', 'WRITE_OFF']));
  ...
  it('filters by location', async () => {
    const res = await request(app.getHttpServer())
      .get(`/api/reports/stock-movement?from=2026-05-01&to=2026-05-31&productId=${productId}&locationId=${toId}`).set(auth()).expect(200);
