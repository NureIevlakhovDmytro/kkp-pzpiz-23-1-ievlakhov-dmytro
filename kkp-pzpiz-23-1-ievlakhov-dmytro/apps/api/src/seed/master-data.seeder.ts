...
const REASONS: { code: WriteOffReasonCode; nameUk: string; nameEn: string }[] = [
  ...
  { code: WriteOffReasonCode.RECEIVING_ERROR, nameUk: 'Помилка приймання', nameEn: 'Receiving error' },
...
export class MasterDataSeeder implements OnModuleInit {
  ...
  async onModuleInit(): Promise<void> {
    for (const r of REASONS) {
      if (!(await this.reasons.findOne({ where: { code: r.code } }))) await this.reasons.save(this.reasons.create(r));
    }
    ...
    if (!base) base = await this.currencies.save(this.currencies.create({ code: 'UAH', name: 'Українська гривня', symbol: '₴' }));
