...
export class UsersService {
  ...
  async findActiveById(id: string): Promise<UserEntity> {
    ...
    if (!user || !user.isActive) throw new AppException(ErrorCode.NOT_FOUND, 'User not found');
    return user;
  }

  async updateProfile(id: string, patch: { fullName?: string; locale?: Locale }): Promise<UserEntity> {
