...
describe('RolesGuard', () => {
  it('allows ADMIN when ADMIN required', () => {
    const reflector = { getAllAndOverride: jest.fn().mockReturnValueOnce(false).mockReturnValueOnce([Role.ADMIN]) } as unknown as Reflector;
  ...
  it('forbids USER when ADMIN required', () => {
    const reflector = { getAllAndOverride: jest.fn().mockReturnValueOnce(false).mockReturnValueOnce([Role.ADMIN]) } as unknown as Reflector;
    const guard = new RolesGuard(reflector);
    expect(() => guard.canActivate(ctxWith({ id: '1', email: 'u', role: Role.USER }))).toThrow(AppException);
  });
